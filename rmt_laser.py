...
class ModelModifier:
    def __init__(self, model_name, svd_method):
    ...
    # Method to update and reduce a model layer
    def update_model_reduce_layer(self, layer_type, layer_number):
        ...
        for name, module in self.model.named_modules():
            if layer_type in name and str(layer_number) in name:
                ...
                # Perform SVD or randomized SVD based on the specified method
                if self.svd_method == "randomized_svd":
                    weights_np = weights.cpu().detach().numpy()
                    U, S, V = randomized_svd(
                        weights_np,
                        n_components=min(weights_np.shape),
                        n_iter="auto",
                        random_state=None,
                    )
                    # Convert U, S, V to PyTorch tensors
                    S = torch.from_numpy(S).to(original_dtype).float()
                    U = torch.from_numpy(U).to(original_dtype).float()
                    V = torch.from_numpy(V).to(original_dtype).float()
                else:
                ...
                # S_reduced = S_reduced.to(original_dtype).float()
            ...
            for layer_type in layer_types:
                ...
                try:
                    ...
                    if loss < min_loss:
                        ...
                        print("*" * 50)
                        print(
                            f"Improved perplexity found: {min_loss} for layer {layer_type} {layer_number}. Total modifications is {mods}"
                        )
                        print("*" * 50)
                    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Modify LLM layers using SVD methods")
    parser.add_argument("model_name", type=str, help="Name of the model")
    parser.add_argument(
        "--method",
        type=str,
        default="randomized_svd",
        choices=["svd", "randomized_svd"],
        help='SVD method to use: "svd" or "randomized_svd" (default: randomized_svd)',
    )

    args = parser.parse_args()

    # Initialize ModelModifier with provided arguments
    modifier = ModelModifier(args.model_name, args.method)
    ...
    loop_check, min_loss = modifier.search_optimal_layer_modification(
        layer_types=[
            "mlp.down_proj",
            "mlp.up_proj",
            "self_attn.q_proj",
